...
def mean_l1_loss(g1, g2):
  with tf.name_scope('mean_l1_loss'):
    return tf.math.reduce_mean(tf.abs(g1-g2))


def mean_l2_loss(g1, g2):
  with tf.name_scope('mean_l2_loss'):
    return tf.math.sqrt(tf.math.reduce_mean(tf.math.square(g1 - g2)))
...
class StyleLoss(object):
  ...
  def __init__(self, style_layers, ema_decay=None,
    ...
    if ema_decay is not None:
      ...
      self.ema = tf.compat.v1.train.ExponentialMovingAverage(decay=ema_decay)
    ...
    self.style_loss = tf.math.add_n(self.style_losses)
